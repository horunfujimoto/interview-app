...
import { Header, LoginForm, Sidebar } from '@/organisms'; // Import Sidebar
import { Alert, Col, Container, Form, Row, Stack, InputGroup as BootstrapInputGroup } from 'react-bootstrap';
import { ArrowRight, BarChart2, CheckCircle, Save, Mail, Lock, Users, Settings, Briefcase, FileText } from 'lucide-react';
    ...
    <Container className="p-4">
      <Header
        ...
        navItems={[
          ...
          { label: '応募者ポータル', href: '#applicant' },
          {
            label: '設定',
            dropdownItems: [
              { label: 'プロフィール設定', href: '#profile-settings' },
              { label: 'アプリ設定', href: '#app-settings' },
            ],
          },
        ]}
        onLogout={() => alert('ログアウトしました！')}
      />
      <Row className="flex-nowrap"> {/* flex-nowrap to prevent sidebar from wrapping on small screens */}
        <Col md={3} className="px-0"> {/* Sidebar column */}
          <Sidebar
            navItems={[
              { label: 'ダッシュボード', href: '#dashboard', icon: BarChart2 },
              { label: '面接管理', href: '#interviews', icon: Briefcase },
              { label: '応募者管理', href: '#applicants', icon: Users },
              {
                label: 'レポート',
                href: '#reports',
                icon: FileText,
                subItems: [
                  { label: '月次レポート', href: '#monthly' },
                  { label: '年次レポート', href: '#yearly' },
                ],
              },
        ...
        <Col md={9} className="py-3 ps-md-4"> {/* Main content column */}
          <main>
            <Breadcrumb>
